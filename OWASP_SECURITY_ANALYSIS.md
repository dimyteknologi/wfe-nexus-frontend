...
|---|---|---|---|
| A01 | Broken Access Control | 🔴 **HIGH** | No middleware route protection; admin pages unguarded |
| A02 | Cryptographic Failures | 🔴 **HIGH** | Hardcoded fallback secret; token in localStorage |
| A03 | Injection | 🟢 **LOW** | No `dangerouslySetInnerHTML` or `eval()` found |
| A04 | Insecure Design | 🟡 **MEDIUM** | No JWT expiry check; session refresh disabled |
| A05 | Security Misconfiguration | 🔴 **HIGH** | Debug mode; hardcoded IPs; no security headers |
| A06 | Vulnerable Components | 🟡 **MEDIUM** | Dev-only dependencies in production bundle |
| A07 | Auth & Session Failures | 🔴 **HIGH** | Open redirect; excessive credential logging |
| A08 | Data Integrity Failures | 🟡 **MEDIUM** | JWT decoded client-side without signature verification |
| A09 | Logging & Monitoring | 🟡 **MEDIUM** | Sensitive data in console logs; no structured logging |
| A10 | SSRF | 🟢 **LOW** | API base URL is fixed; no dynamic URL construction |
